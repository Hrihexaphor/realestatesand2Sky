...
const DeveloperPage = () => {
  ...
  const exportToExcel = async () => {
    try {
      ...
      const colWidths = [
        { wch: 6 }, { wch: 20 }, { wch: 25 }, { wch: 30 }, { wch: 15 },
        { wch: 30 }, { wch: 15 }, { wch: 15 }, { wch: 15 }, { wch: 12 }, { wch: 12 }
      ];
      ws['!cols'] = colWidths;
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4">
      <h1 className="text-3xl font-bold mb-8 text-gray-800">Developer Management</h1>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          {/* Logo Upload */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">Developer Logo</label>
            <div className="flex gap-4">
              ...
              {previewImage && (
                <div className="w-1/2">
                  <img src={previewImage} alt="Preview" className="h-20 w-20 object-contain border rounded" />
                  <button 
                    type="button" 
                    onClick={() => {
                      setPreviewImage(null);
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              {editingId && <p className="mt-1 text-xs text-gray-500">Company name cannot be edited</p>}
            </div>
          ...
          <div className="flex justify-end space-x-3">
            ...
            >
              {submitting ? 'Processing...' : (editingId ? 'Update Developer' : 'Add Developer')}
            </button>
          </div>
        </form>
      </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="grid md:grid-cols-2 gap-2 text-sm text-gray-600 mb-3">
                      <div><strong>Company:</strong> {dev.company_name}</div>
                      <div><strong>Email:</strong> {dev.contact_email}</div>
                      <div><strong>Phone:</strong> {dev.phone_number}</div>
                      {dev.partial_amount && <div><strong>Amount:</strong> ₹{dev.partial_amount}</div>}
                      {(dev.address || dev.city || dev.state) && (
                        <div className="md:col-span-2">
                          <strong>Address:</strong> {[dev.address, dev.city, dev.state].filter(Boolean).join(', ')}
                        </div>
                      )}
                    </div>
                    
                    <div className="text-xs text-gray-500">
                      {dev.created_at && <span>Created: {new Date(dev.created_at).toLocaleDateString()}</span>}
                      {dev.updated_at && <span className="ml-4">Updated: {new Date(dev.updated_at).toLocaleDateString()}</span>}
                    </div>
                  </div>
