        ...
        <div className="mb-8">
          <div className="flex items-center mb-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Manage your {properties.length} properties efficiently</p>
            </div>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-gradient-to-r from-gray-800 to-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-white uppercase">Details</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedProperties.map((property, index) => (
                  <>
                    {/* Main Row */}
                    <tr key={property.property_id || property.id} className="hover:bg-gray-50 transition-colors duration-150">
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex justify-center space-x-1">
                          ...
                          <button 
                            onClick={() => onDelete(property.property_id || property.id)} 
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex justify-center space-x-1">
                          <button 
                            onClick={() => navigate(`/dashboard/property/${property.property_id || property.id}/add-faq`)} 
                            className="py-1 px-2 text-xs text-indigo-600 bg-indigo-100 rounded hover:bg-indigo-200 transition-all font-medium"
                          >
                            Add
                          </button>
                          <button 
                            onClick={() => navigate(`/dashboard/property/${property.property_id || property.id}/faqs`)} 
                      ...
                      <td className="px-4 py-4 text-sm text-gray-600">
                        <div>
                          <div className="font-medium">{property.property_category_name || '—'}</div>
                          {property.property_subcategory_name && (
                            <div className="text-xs text-gray-500">{property.property_subcategory_name}</div>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-4 text-sm font-medium">
                        <button
                          onClick={() => toggleExpanded(property.property_id || property.id)}
                          ...
                          <div className="border-l-4 border-amber-500 pl-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                              ...
                              <div className="space-y-2">
                                <h5 className="font-medium text-gray-700 text-sm">Basic Information</h5>
                                <div className="space-y-1 text-sm">
                                  <div><span className="text-gray-500">Developer:</span> {property.developer_name || '—'}</div>
                                  <div><span className="text-gray-500">Transaction:</span> {property.transaction_type || '—'}</div>
                                  <div><span className="text-gray-500">Possession:</span> {property.possession_status || '—'}</div>
                                  <div><span className="text-gray-500">Furnishing:</span> {property.furnished_status || '—'}</div>
                                </div>
                              </div>

                              {/* Property Details */}
                              <div className="space-y-2">
                                <h5 className="font-medium text-gray-700 text-sm">Property Details</h5>
                                <div className="space-y-1 text-sm">
                                  ...
                                  <div><span className="text-gray-500">Carpet Area:</span> {property.carpet_area ? `${property.carpet_area} sq ft` : '—'}</div>
                                  <div><span className="text-gray-500">Built-up Area:</span> {property.built_up_area ? `${property.built_up_area} sq ft` : '—'}</div>
                                </div>
                              </div>

                              {/* Additional Info */}
                              <div className="space-y-2">
                                <h5 className="font-medium text-gray-700 text-sm">Additional Information</h5>
                                <div className="space-y-1 text-sm">
                                  <div><span className="text-gray-500">Posted:</span> {property.created_at ? new Date(property.created_at).toLocaleDateString() : '—'}</div>
                                  <div><span className="text-gray-500">Updated:</span> {property.updated_at ? new Date(property.updated_at).toLocaleDateString() : '—'}</div>
                                  {property.description && (
                                    <div className="mt-2">
                                      <span className="text-gray-500">Description:</span>
                                      <p className="text-gray-600 text-xs mt-1">{property.description.substring(0, 100)}...</p>
                                    </div>
                                  )}
                                </div>
                              </div>
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      ...
                      <span className="font-medium">{Math.min(startIndex + itemsPerPage, sortedProperties.length)}</span> of{' '}
                      <span className="font-medium">{sortedProperties.length}</span> properties
                      {searchTerm && <span className="text-gray-500"> (filtered from {properties.length} total)</span>}
                    </p>
                  </div>
                  <div>
