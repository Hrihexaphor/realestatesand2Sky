...
import {
  ...
  getAllFeaturedIds,
  checkIfFeatured,
  getActiveFeaturedPropertiesLite,
  getFeaturedProperties,
  updateFeaturedProperty,
  getFeaturedPropertyDetails,
  getFeaturedPropertiesWithCities,
} from "../services/featuredProperty.js";
...
router.post("/addtofeatured", async (req, res) => {
  ...
  try {
    ...
    if (isFeatured) {
      return res
        .status(400)
        .json({ message: "This property is already featured" });
    }

    const result = await addToFeatured(
      property_id,
      start_date,
      end_date,
      cities
    ...
    res
      .status(500)
      .json({ error: "Failed to add to featured", details: err.message });
  }
});
...
router.put("/updatefeatured/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Failed to update featured property",
        details: err.message,
      });
  }
});
...
router.get("/getfeatured/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Failed to fetch featured property",
        details: err.message,
      });
  }
});
// Add this route to your routes file
router.get("/featured-with-cities", async (req, res) => {
  try {
    const featuredProperties = await getFeaturedPropertiesWithCities();
    res.status(200).json(featuredProperties);
  } catch (err) {
    res
      .status(500)
      .json({
        error: "Failed to fetch featured properties with cities",
        details: err.message,
      });
  }
});
...
router.delete("/featured/:property_id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Failed to remove featured property",
        details: err.message,
      });
  }
});
...
router.get("/featuredids", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch featured IDs", details: err.message });
  }
});
