...
import {
  ...
  getAllProperties,
  deletePropertyById,
  getReadyToMoveProperties,
  sendNewPropertyEmails,
} from "../services/propertyService.js";
  ...
  async (req, res) => {
    try {
      ...
      if (!basic.property_type) {
        return res
          .status(400)
          .json({ error: "Missing required property type (subcategory)" });
      }

      // Validate that property_type is a valid subcategory for the given category_id
      try {
        const validSubcategories = await getSubcategoriesByCategoryId(
          basic.category_id
        );
        const isValidSubcategory = validSubcategories.some(
          (subcategory) => subcategory.id === parseInt(basic.property_type)
        );

        if (!isValidSubcategory) {
          return res.status(400).json({
            error:
              "Invalid property type. The selected property type does not belong to the selected category.",
          });
        }
      } catch (err) {
      ...
      if (configurations && configurations.length > 0) {
        ...
        let configFileMeta;
        try {
          configFileMeta = req.body.configFileMeta
            ? JSON.parse(req.body.configFileMeta)
            : [];
        } catch (err) {
          console.warn(
            "Invalid configFileMeta format, treating as empty array:",
            err
          );
          configFileMeta = [];
        }
        ...
        await insertPropertyConfigurations(
          property.id,
      ...
      if (req.files?.documents?.length > 0) {
        ...
        const documents = req.files.documents.map((file) => {
          const meta = documents_metadata.find(
