...
import {
  ...
  updateDeveloper,
  deleteDeveloper,
} from "../services/developerServices.js";
  ...
  uploadDeveloperLogo.single("developerImage")(req, res, async function (err) {
    ...
    try {
      ...
      const developerData = {
        ...
        partial_amount:
          req.body.partial_amount && req.body.partial_amount.trim() !== ""
...
router.put(
  "/developer/:id",
  uploadDeveloperLogo.single("developerImage"),
  async (req, res) => {
    const { id } = req.params;
    const developerData = req.body;

    if (req.file) {
      developerData.developer_logo = req.file.path; // Cloudinary URL
    }
