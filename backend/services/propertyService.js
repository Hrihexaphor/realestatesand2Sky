...
export async function insertPropertyDetails(property_id, details) {
 try {
   const fields = [
     'project_area', 'num_of_units', 'project_rera_id', 'city', 'location', 
   ...
   const existingFields = fields.filter(field => details[field] !== undefined && details[field] !== null);
   ...
   const query = `INSERT INTO property_details (property_id, ${existingFields.join(', ')}) VALUES ($1, ${placeholders})`;
...
export async function deletePropertyById(id) {
 try {
   ...
   const property = await pool.query(`SELECT id FROM property WHERE id = $1`, [id]);
...
export const searchProperty = async (filters) => {
  ...
  const propertyColumns = ['id', 'expected_price', 'property_type', 'possession_status', 'furnished_status'];
  const detailsColumns = ['bedrooms', 'bathrooms', 'built_up_area', 'city', 'locality'];
  ...
  try {
    ...
    const countQuery = query.split('ORDER BY')[0].replace('SELECT p.*, d.*', 'SELECT COUNT(*)');
